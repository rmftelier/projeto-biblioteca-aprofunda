...
export class UpdateBookByIdController {
  async handle(req: Request, res: Response): Promise<Response> {
    const { id } = req.params;
    const { title, author, publishedYear, format, pages, genres, language } = req.body;

    try {
      const updateBook = new UpdateBook(bookRepository);
      const book = await updateBook.execute(id, { title, author, publishedYear, format, pages, genres, language });
